...
(
(SELECT CustomerID, FirstName, LastName, Address FROM Customers WHERE Customers.FirstName = 'Loretta' AND Customers.LastName = 'Hundey'), 
(SELECT Name FROM Dishes WHERE Dishes.Name IN ('Tropical Blue Smoothie','House Salad','Mini Cheeseburgers')), '2023-09-20 14:00',(SELECT SUM(Price) FROM Dishes WHERE Dishes.Name IN ('Tropical Blue Smoothie','House Salad','Mini Cheeseburgers')) )