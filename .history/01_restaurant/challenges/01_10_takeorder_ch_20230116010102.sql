...
SELECT SUM(PRICE) FROM Dishes JOIN OrdersDishes ON OrdersDishes.DishID = Dishes.DishID WHERE ORDER

--Insert it into delivery databases 
UPDATE CustomerDelivery  SET Items = 3 WHERE FirstName = 'Loretta'
UPDATE CustomerDelivery SET Total = SUM(Price) FROM Dishes WHERE Dishes.Name in ('Tropical Blue Smoothie', 'Mini Cheeseburgers', 'House Salad'
